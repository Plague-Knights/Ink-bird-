...
// Three.js/R3F ships a lot of client-only globals (WebGL, canvas), so the
// scene is loaded dynamically with SSR disabled. The fallback mirrors the
// canvas wrapper so the layout doesn't jump before hydration.
const DiveScene = dynamic(() => import("@/components/DiveScene").then((m) => m.DiveScene), {
  ssr: false,
  loading: () => <div className="dive-canvas-wrap"><div className="dive-canvas" /></div>,
});
