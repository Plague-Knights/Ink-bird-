...
import { isHex32 } from "@/lib/dive";
import {
  cannonEventsFor,
  cannonOutcomeIndexOf,
  cannonPayoutWei,
  totalMultiplierBps,
} from "@/lib/cannon";
...
// POST /api/cannon/play
// Body: { roundId, userSalt: "0x…32b", betWei: "<string>" }
// Resolves a committed round: locks the bet + salt, reveals the seed,
// derives the deterministic event sequence, and credits the payout to
// the caller's DiveBalance. The event sequence is persisted alongside
// the round so the client can replay the launcher animation and so
// /api/cannon/verify can show the full audit trail.
export async function POST(req: Request) {
  ...
  let body: { roundId?: unknown; userSalt?: unknown; betWei?: unknown };
  try {
    body = await req.json();
  } catch {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const round = await tx.cannonRound.findUnique({ where: { id: body.roundId as string } });
      ...
      const index = cannonOutcomeIndexOf(seedHex, saltHex);
      const events = cannonEventsFor(index);
      const totalBps = totalMultiplierBps(events);
      const payout = cannonPayoutWei(totalBps, betWei);
      ...
      const updated = await tx.cannonRound.update({
        where: { id: round.id },
        data: {
          betWei: betWei.toString(),
          userSalt: saltHex,
          outcomeIndex: index,
          // Prisma serializes as Json — clone so the frozen dataset
          // doesn't leak into the driver.
          eventSeq: events.map((e) =>
            e.kind === "blot" ? { kind: "blot", value: e.value } : { kind: "hazard" },
          ),
          totalMultiplierBps: totalBps,
    ...
    return NextResponse.json({
      ...
      events: result.round.eventSeq,
      totalMultiplierBps: result.round.totalMultiplierBps,
    ...
    if (status === 500) console.error("[cannon/play] failure", e);
